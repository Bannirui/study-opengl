                            ...
                            size_t indexSz)
{
    ...
    for (const auto& desc : kVBOTable)
    {
        if (Has(layout.attrs, desc.types))
        {
    ...
    uint32_t index  = 0;
    {
        ...
        glVertexAttribPointer(index, static_cast<uint32_t>(layout.posDim), GL_FLOAT, GL_FALSE, stride,
    ...
    for (const auto& desc : kVBOTable)
    {
        ...
        glEnableVertexAttribArray(index);
        glVertexAttribPointer(index, desc.components, GL_FLOAT, GL_FALSE, stride, reinterpret_cast<void*>(offset));
